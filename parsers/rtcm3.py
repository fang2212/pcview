    ...
    def read_from_file(self, filename):
        ...
        while current_Line < (len(lines) - 1):
            ...
            if lines[current_Line].startswith("INT:"):
                ...
                name = line.strip()
                self.fields.append(
                    {'type': "INT", 'name': name, 'df_number': df_number, 'bitlength': bitlength, 'value': None})
                ...
                name = line.strip()
                self.fields.append(
                    {'type': "UINT", 'name': name, 'df_number': df_number, 'bitlength': bitlength, 'value': None})
                ...
                name = line.strip()
                self.fields.append(
                    {'type': "REPEAT", 'name': name, 'df_number': df_number, 'bitlength': bitlength, 'value': None})
...
class RTCM3:
    def __init__(self, default_output_level=2):
    ...
    def add_data(self, data):
        ...
        self.buffer += data
        # print('added data to rtcm3 parser', len(self.buffer))
    ...
    def process_data(self, dump_decoded=False):
        ...
        if CRC == Computed_CRC:
            ...
            packet_data = self.packet[
                          RTCM3_First_Data_Location:self.packet_Length + RTCM3_First_Data_Location]  # The packet
            self.buffer = self.buffer[self.packet_Length + RTCM3_Min_Size:]
            self.undecoded = bytearray()
            self.decode(self.packet_ID, packet_data)
            # print('rcvd rtcm3 packet', self.packet_ID, self.packet_Length)
            return Got_Packet
        else:
            #            print("Invalid")
    ...
    def dump(self, dump_undecoded=False, dump_status=False, dump_decoded=False, dump_timestamp=False):
        if self.Dump_Levels[self.packet_ID]:
            ...
            if self.packet_ID in self.commands:
                ...
                # print("")
                r['cmd_name'] = self.commands[self.packet_ID].Command_Name
                r['id'] = self.packet_ID
                return r
...
if __name__ == "__main__":
    ...
    for file in files:
        ...
        new_file = file + '.json'
        sys.stderr.write("Loaded Command: " + "{0}:{1}\n".format(rtcm3_Defs.Command_ID, rtcm3_Defs.Command_Name))
        commands[rtcm3_Defs.Command_ID] = rtcm3_Defs
        # json.dump(rtcm3_Defs, open(new_file, 'w'))
    pickle.dump(commands, open('/home/nan/workshop/git/RTCM3/DEFS/all.pickle', 'wb'))
