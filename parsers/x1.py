...
vision_color = CVColor.Peru
fusion_color = CVColor.Chocolate


def parse_x1(id, data, ctx=None):
    """
    完整的x1数据解析，包含融合跟单目的结果
    :param id:
    :param data:
    :param ctx:
    :return:
...
def parse_x1_fusion(id, data, ctx=None):
    """
    x1融合数据，另外根据业务需求加了车道线解析
    :param id:
    :param data:
    :param ctx:
    :return:
    ...
    # fusion
        ...
        if index not in x1_fusion_lane:
            x1_fusion_lane[index] = {}

        # lane Data A
        tmp1 = 'Lane' + '%01d' % (index + 1) + '_Type'
        if tmp1 in r:
            x1_fusion_lane[index] = dict()
            x1_fusion_lane[index]['Lane_Type'] = r['Lane' + '%01d' % (index + 1) + '_Type']
            x1_fusion_lane[index]['Quality'] = r['Lane' + '%01d' % (index + 1) + '_Quality']
            x1_fusion_lane[index]['a0'] = r['Lane' + '%01d' % (index + 1) + '_Position']
            x1_fusion_lane[index]['a2'] = r['Lane' + '%01d' % (index + 1) + '_Curvature']
            x1_fusion_lane[index]['a3'] = r['Lane' + '%01d' % (index + 1) + '_CurvatureDerivative']
            x1_fusion_lane[index]['WidthMarking'] = r['Lane' + '%01d' % (index + 1) + '_WidthMarking']

        # lane Data B
        tmp1 = 'Lane' + '%01d' % (index + 1) + '_HeadingAngle'
        if tmp1 in r:
            if not x1_fusion_lane[index]:
                return None

            x1_fusion_lane[index]['a1'] = r['Lane' + '%01d' % (index + 1) + '_HeadingAngle']
            x1_fusion_lane[index]['ViewRangeStart'] = r['Lane' + '%01d' % (index + 1) + '_ViewRangeStart']
            x1_fusion_lane[index]['range'] = r['Lane' + '%01d' % (index + 1) + '_ViewRangeEnd']
            x1_fusion_lane[index]['LineCrossing'] = r['Lane' + '%01d' % (index + 1) + '_LineCrossing']
            x1_fusion_lane[index]['LineMarkColor'] = r['Lane' + '%01d' % (index + 1) + '_LineMarkColor']

            x1_fusion_lane[index]['type'] = 'lane'
            x1_fusion_lane[index]['sensor'] = 'x1'
            x1_fusion_lane[index]['id'] = index

        if id == 0x5f7:
            res = []
            for lane in x1_fusion_lane:
                res.append(x1_fusion_lane[lane])
            # res = x1_lane.copy()
            x1_fusion_lane.clear()
            # print(res)
            return res


def parse_x1_vision(id, data, ctx=None):
    """
    x1单目数据，另外根据业务需求加了车道线解析
    :param id:
    :param data:
    :param ctx:
    :return:
