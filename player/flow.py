...
class FlowPlayer(object):
    def __init__(self, cfg={}):
        ...
        self.is_big_car = cfg.get('is_big_car', True)
    ...
    def draw(self, mess, img):
        ...
        if 'camera' in mess:
            ...
            if 'finish_time_us' in mess:
                ...
                total_ts = -1
                lane_ts = mess.get('lane_finish_time', 0)
                vehicle_ts = mess.get('vehicle_finish_time', 0)
                tsr_ts = mess.get('tsr_finish_time', 0)
                if lane_ts:
        ...
        if 'pedestrians' in mess:
            ...
            for i, obj in enumerate(res_list):
                BaseDraw.draw_obj_rect(img, obj.get('detect_box'), CVColor.Cyan, 1)
                if obj.get('bike_box'):
                    BaseDraw.draw_obj_rect(img, obj.get('bike_box'), CVColor.Yellow, 1)
                pos = obj.get('regressed_box')
                color = CVColor.Pink if obj.get('is_key') else CVColor.Green
        ...
        if 'vehicle_hit_list' in mess:
            ...
            for i, obj in enumerate(res_list):
                BaseDraw.draw_obj_rect(img, obj.get('det_rect'), CVColor.Red, 1)

        if 'ped_hit_list' in mess:
            res_list = mess['ped_hit_list']
            for i, obj in enumerate(res_list):
                BaseDraw.draw_obj_rect(img, obj.get('det_rect'), CVColor.Green, 1)

        if 'tsr_trace_res_list' in mess:
            res_list = mess['tsr_trace_res_list']
            for i, obj in enumerate(res_list):
                BaseDraw.draw_obj_rect(img, obj.get('reg_rect'), CVColor.Red, 1)
                pos = obj.get('reg_rect')
                tsr_class = obj.get('tsr_class')
                tsr_id = obj.get('tsr_id')
                track_cnt = obj.get('track_cnt')
                para_list = [
                    'class:' + str(tsr_class),
                    'id:' + str(tsr_id),
        ...
        if mess.get('ihc_signal', 0):
            ihc_list = mess.get('ihc_fusion_result', [])

            para_list = ['ihc_signal:' + str(mess.get('ihc_signal'))]
            # BaseDraw.draw_single_info(img, (930, 0), 180, 'ihc', para_list)

            ihc_detect_region = (640, 320)
            h, w, d = img.shape
            p1 = [w // 2 - ihc_detect_region[0] // 2, h // 2 - ihc_detect_region[1] // 2]
            p2 = [w // 2 + ihc_detect_region[0] // 2, h // 2 + ihc_detect_region[1] // 2]

            if self.is_big_car:
                p1[1] += 20
                p2[1] += 20
            # print(p1, p2)
            BaseDraw.draw_head_info(img, p1, ['ihc_detect_region'])

            BaseDraw.draw_rect(img, tuple(p1), tuple(p2), CVColor.White)

            for ihc in ihc_list:
                color_list = [CVColor.White, CVColor.Red, CVColor.Yellow]

                hit_type = ihc['hit_type']

                color = color_list[hit_type]
                BaseDraw.draw_obj_rect(img, ihc['det_rect'], color)
                para_list = ['hit_type', 'trace_id', 'vertical_dist', "horizontal_dist"]

                ihc['vertical_dist'] = "%.2f" % ihc['vertical_dist']
                ihc["horizontal_dist"] = "%.2f" % ihc["horizontal_dist"]
                para_list = [key + ":" + str(ihc[key]) for key in para_list]
                BaseDraw.draw_head_info(img, ihc["det_rect"][:2], para_list)
