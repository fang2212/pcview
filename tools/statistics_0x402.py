...
class Statistics:
    ...
    def __init__(self, log_path, save_path=None, can="CAN0"):
        self.log_path = log_path
        self.save_path = save_path
        self.can = can
        self.log_dir_name = os.path.split(os.path.dirname(log_path))[-1]
        self.log_dir = os.path.dirname(log_path)

        self.all_ts = []                            # 0x402的信号ts列表
        self.frame_ts = []                          # 有效frame_index信号ts列表
    ...
    def init(self):
        ...
        # 初始化表头
        excel_header = ["信号", '平均值', '最小值', '最大值', '标准差']
    ...
    def run(self):
        ...
        with open(self.log_path) as f:
            ...
            for line in lines:
                ...
                if self.can == cols[2] and "0x402" == cols[3]:
                    ts = float(cols[0]) + float(cols[1]) / 1000000
                    # self.all_ts.append(ts)

                    # 根据dbc文件格式，将报文前8位截取后组成数组
                    data = [cols[4][i:i+2] for i in range(0, len(cols[4][:8]), 2)]
                    # 根据dbc文件格式，将报文重新排序
                    data.reverse()
                    # 解出二进制id
                    frame_id = min(int(''.join(data), 16), 4294967295)
                    self.frame_ts.append({"ts": ts, "fid": frame_id})
                    test_list.append(frame_id)
    ...
    def statistics_other(self):
        file_name = os.path.join(self.save_path, f"{self.log_dir_name}_0x402数据接收")
        print("file:", file_name)
        self.render_can([self.frame_ts], file_name=file_name, title=f"")
    ...
    def render_can(self, data_list, title="", file_name="统计图表.png"):
        ...
        for can_id_data in data_list:
            ...
            # 写入到表格对象中
            col_data = ["0x402", avg_interval, min_interval, max_interval, std_interval]
            ...
            # plt.plot(interval_list)  # s表示面积，marker表示图形
            plt.plot(timestamp_list[1:], interval_list)  # s表示面积，marker表示图形
    ...
    def export_excel(self):
        excel_filename = os.path.join(self.save_path, "0x402接收数据统计表.xlsx")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--can', '-c', help='can设备名', default="CAN0")
    ...
    for path in tqdm(log_path_list):
        task = Statistics(path, save_path=args.save, can=args.can)
        task.run()
    logger.warning("运行结束")
